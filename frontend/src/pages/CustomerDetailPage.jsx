...
export default function CustomerDetailPage() {
  ...
  const loadAiInsight = useCallback(async () => {
    ...
    setAiError(null);
    try {
      const insight = await getAiSuggestions(id);
  ...
  return (
    <>
      ...
      {/* AI Insights (Gemini) */}
      <GlassCard style={{ marginBottom: 20 }}>
        <div style={{ padding: '16px 20px', borderBottom: aiInsight ? '1px solid var(--border)' : 'none', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
          <div style={{ fontWeight: 700, fontSize: 15, color: 'var(--text-primary)' }}>AI Portfolio Insights</div>
          <Button variant="secondary" size="sm" onClick={loadAiInsight} disabled={aiLoading}>
            {aiLoading ? 'Analysing…' : aiInsight ? 'Refresh Insights' : 'Generate Insights'}
          </Button>
        </div>
        {aiError && (
          <div style={{ padding: '14px 20px' }}>
            <p style={{ color: 'var(--danger)', fontSize: 13.5 }}>{aiError}</p>
          </div>
        )}
        {aiInsight && (
          <div style={{ padding: '16px 20px' }}>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 8 }}>
              <Badge label={aiInsight.riskLevel} variant={aiInsight.riskLevel === 'HIGH' ? 'High' : aiInsight.riskLevel === 'MEDIUM' ? 'Medium' : 'Low'} />
              <span style={{ fontSize: 11.5, color: 'var(--text-tertiary)' }}>
                {aiInsight.source === 'AI' ? 'Generated by Gemini' : 'Rule-based (AI unavailable)'}
              </span>
            </div>
            <p style={{ fontSize: 13.5, color: 'var(--text-secondary)', lineHeight: 1.65, marginBottom: aiInsight.suggestions?.length ? 12 : 0 }}>
              {aiInsight.summary}
            </p>
            {aiInsight.suggestions?.length > 0 && (
              <ul style={{ margin: 0, paddingLeft: 18, display: 'flex', flexDirection: 'column', gap: 6 }}>
                {aiInsight.suggestions.map((s, i) => (
                  <li key={i} style={{ fontSize: 13.5, color: 'var(--text-secondary)', lineHeight: 1.6 }}>{s}</li>
                ))}
              </ul>
            )}
          </div>
